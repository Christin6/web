...
- **Authentication:** JSON Web Tokens (JWT). Tokens are issued upon login and validated via custom middleware on every protected API route. Passwords are hashed with bcrypt before storage. The auth flow stores the token securely and rehydrates it on page load so sessions persist across browser refreshes.
- **Security:** 
